<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT p.*, u.username, u.avatar, c.name as `category` FROM `post_list` p inner join category_list c on p.category_id = c.id inner join `df_users` u on p.user_id = u.id where p.id= '{$_GET['id']}'");
    if($qry->num_rows > 0){
        ...
        <div class="card rounded-1 shadow">
            ...
            <div class="card-body">
                <div class="contrain-fluid">
                    ...
                    <div class="list-group comment-list mb-3 rounded-3">
                        <?php 
                        $comments = $conn->query("SELECT c.*, u.username, u.avatar FROM `comment_list` c inner join `df_users` u on c.user_id = u.id where c.post_id ='{$id}' order by abs(unix_timestamp(c.date_created)) asc ");
